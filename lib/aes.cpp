...
uint8_t AES::rcon[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
...
void AES::setKey(const uint8_t (&key)[16]) {
    mNb = 4;
    mNk = 4;
    mNr = 10;
    memcpy(mKey, key, (size_t) mNk * 4);
}
void AES::setKey(const uint8_t (&key)[24]) {
    mNb = 4;
    mNk = 6;
    mNr = 12;
    memcpy(mKey, key, (size_t) mNk * 4);
}
void AES::setKey(const uint8_t (&key)[32]) {
    mNb = 4;
    mNk = 8;
    mNr = 14;
    memcpy(mKey, key, (size_t) mNk * 4);
...
void AES::addRoundKey(uint8_t state[][4], uint8_t w[][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] ^= w[i][j];
        }
...
//calculate the least significant uint8_t only for we only need
//the least significant byte
uint8_t AES::GF28Multi(uint8_t s, uint8_t a) {
    uint8_t t[4];
    uint8_t result = 0;
    t[0] = s;
...
void AES::encrypt_block(const uint8_t (&data)[16], uint8_t (&out)[16]) {
    uint8_t state[4][4];
    uint8_t key[4][4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
...
void AES::decrypt_block(const uint8_t (&data)[16], uint8_t (&out)[16]) {
    uint8_t state[4][4];
    uint8_t key[4][4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
